...
const VisionPage: React.FC<VisionPageProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#022c22] text-white selection:bg-emerald-500/30 selection:text-emerald-200 relative overflow-x-hidden">
      ...
      >
        {/* Swapped for a deep-sea seaweed texture/macro image to avoid human-centric focus */}
        <img 
          src="https://images.unsplash.com/photo-1516053353285-399f743fef88?auto=format&fit=crop&q=80&w=2000" 
          className="w-full h-full object-cover blur-sm grayscale block bg-emerald-950"
          alt="Macro seaweed background"
        />
        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-16 relative">
          {[
            ...
            <div 
              ...
              style={{ transform: typeof window !== 'undefined' && window.innerWidth > 1024 ? `translateY(${scrollY * (0.015 * (i + 1))}px)` : 'none' }}
            >
